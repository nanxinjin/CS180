...
public class View extends JPanel{
    ...
    private void drawLocation() {
        ...
        while (it.hasNext()) {
            ...
            while (it2.hasNext()) {
                ...
                String locations = model.getRequester2Location().get(key);
                String coordinates = model.getLocation2Coordinate().get(locations);
                
                if(Coordinate.equals(coordinates)) {
                	drawName(requesterImage, key, x, y);
                	y+=dy;
                }
            ...
            while (it3.hasNext()) {
                ...
                //using drawName method  
                String locations2 = model.getVolunteer2Location().get(key);
                String coordinates2 = model.getLocation2Coordinate().get(locations2);
                
                if(coordinates2.equals(Coordinate)) {
                	drawName(volunteerImage, key, x, y);
                	y+=dy;
    ...
    private void drawIntransit() {
        ...
        while (it.hasNext()) {   
            ...
            int lineWidth = g2.getFontMetrics().getHeight();
            ...
            String [] stores = mover.split(" ");
            String moverName = stores[0];
            
            //TODO Draws the mover name at the obtained position using drawString method 
            g2.drawString(moverName, Math.round(x * scaleWidth), Math.round(y * scaleHeight) + lineWidth - 3);
            ...
            String startLocation = model.getVolunteer2Location().get(moverName);
            
            //TODO Obtains location coordinates of the Building using getLocation2Coordinate() method in model
            String coordinates3 = model.getLocation2Coordinate().get(startLocation);
            String [] coordinatesStore = coordinates3.split(" ");
            
            //TODO Splits the location String obtained into coordinates using parseInt() and store in two different variables 
            int x0 = Integer.parseInt(coordinatesStore[0]);
            int y0 = Integer.parseInt(coordinatesStore[1]);
            
            //TODO Stores destination Building name into a String variable
            String destinations = stores[1];
            //TODO gets coordinates of the building using getLocation2Coordinate() method
            String coordinatesB = model.getLocation2Coordinate().get(destinations);
            //TODO Splits the obtained String to save the coordinates in two int variables
            String [] obtains = coordinatesB.split(" ");
            int x1 = Integer.parseInt(obtains[0]);
            int y1 = Integer.parseInt(obtains[1]);
            g2.setStroke(dashed); //sets drawing style to shaded line
            
            //TODO: Draws a line using GraphicsObject.drawLine() method 
            g2.drawLine((Math.round(x0 * scaleWidth) + DIAMETER / 2),
            						(Math.round(y0 * scaleHeight) + DIAMETER / 2),
            						(Math.round(x1 * scaleWidth) + DIAMETER / 2),
            						(Math.round(y1 * scaleHeight) + DIAMETER / 2));
